...
export default class Statements {
  ...
  static CSVRegex = /^.+\.csv$/gi;

  mapStatements (data) {
    if (data[0] === undefined || (data[0].Date !== undefined && data[0].Date.length > 0 && data[0].Description !== undefined)) {
      return data;
    }
    const outputData = [];
    // Monzo style statements
    if (data[0].created) {
      data.forEach((row) => {
        outputData.push({
    ...
    // Barclays style statements
    else if (data[0].Subcategory) {
      data.forEach((row) => {
        outputData.push({
          Date: row.Date,
          Description: row.Memo.slice(0, 22),
          Value: row.Amount,
          Balance: null,
