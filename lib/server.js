...
export default class Server {
  ...
  statementsRequest (socket, opts) {
    ...
    .then((files) => socket.emit('statements:receive', null, files))
    .error((err)  => socket.emit('error', err));
  }

  transactionsRequest (socket, opts) {
  ...
  initStatementsSiofu (socket) {
    ...
    uploader.on('complete', (event) => {
      ...
      Transactions.setCache().finally(() => {
        ...
        this.transactionsRequest(socket, {});
      });
    });
  }
  ...
  constructor (opts) {
    ...
    statements.on('connection', (socket) => {
      ...
      socket.on('statements:request', (opts, callback) => this.statementsRequest(socket, opts));
    });

    let transactions = io.of('/transactions');
    transactions.on('connection', (socket) => {
      socket.on('transactions:request', (opts, callback) => this.transactionsRequest(socket, opts));
    });
