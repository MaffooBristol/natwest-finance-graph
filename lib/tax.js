...
export default class Tax {
  ...
  displayTable () {
    ...
    const output = _.map(this.data, (val) => [chalk.cyan(Base.formatCurrency(val.Value)), (val.Type || ''), val.Date, val.Description]);
  ...
  displayTotal () {
    ...
    if (moment(this.taxYear.filters.endDate, Base.dateFormat).isAfter(moment())) {
      ...
      const proRataDays = moment().diff(moment(this.taxYear.filters.startDate, Base.dateFormat), 'days');
